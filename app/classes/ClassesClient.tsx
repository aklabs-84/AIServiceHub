...
type LocationFilter = 'all' | 'online' | 'offline' | 'hybrid';

export default function ClassesClient({ initialCourses }: ClassesClientProps) {
  const { isAdmin } = useAuth();
  const [pricing, setPricing] = useState<FilterType>('all');
  ...
  const filtered = useMemo(() => {
    ...
    if (location !== 'all') result = result.filter(c => c.locationType === location);
    return result;
  }, [initialCourses, pricing, location]);
  ...
  const counts = useMemo(() => ({
    ...
    online: initialCourses.filter(c => c.locationType === 'online').length,
    offline: initialCourses.filter(c => c.locationType === 'offline').length,
    hybrid: initialCourses.filter(c => c.locationType === 'hybrid').length,
  }), [initialCourses]);
    ...
    <main className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <section className="relative pt-20 pb-16 px-4 overflow-hidden">
        ...
        <div className="relative container mx-auto max-w-6xl">
          ...
          {isAdmin && (
            <Link
              href="/classes/new"
              className="absolute top-0 right-0 flex items-center gap-2 px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-700 text-white font-bold text-sm shadow-lg shadow-violet-200 dark:shadow-violet-900/30 transition-all"
            >
      ...
      <section className="sticky top-16 md:top-20 z-30 bg-white/80 dark:bg-gray-950/80 backdrop-blur-xl border-b border-gray-100 dark:border-gray-800">
        <div className="container mx-auto max-w-6xl px-4 py-3 flex flex-wrap gap-2">
          {/* 가격 필터 */}
          {([['all', '🔍 전체', counts.all], ['free', '🆓 무료', counts.free], ['paid', '💎 유료', counts.paid]] as [FilterType, string, number][]).map(([key, label, count]) => (
            <button
              key={key}
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all border ${
                pricing === key
                  ...
                  : 'bg-gray-50 dark:bg-gray-900 border-gray-100 dark:border-gray-800 text-gray-500 dark:text-gray-400 hover:border-violet-300 dark:hover:border-violet-700'
              }`}
            >
              {label}
              <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${pricing === key ? 'bg-white/20 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-500'}`}>
                {count}
              </span>
            </button>
          ))}
          ...
          {/* 방식 필터 */}
          {([['all', '전체'], ['online', '🖥️ 온라인'], ['offline', '🏫 오프라인'], ['hybrid', '🔀 혼합']] as [LocationFilter, string][]).map(([key, label]) => (
            <button
              key={key}
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all border ${
                location === key
                  ...
                  : 'bg-gray-50 dark:bg-gray-900 border-gray-100 dark:border-gray-800 text-gray-500 dark:text-gray-400 hover:border-gray-300 dark:hover:border-gray-600'
              }`}
            >
              {label}
            </button>
