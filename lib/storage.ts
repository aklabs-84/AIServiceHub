...
import { AppAttachment } from '@/types/app';
...
export async function uploadAppAttachment(file: File, idToken: string): Promise<AppAttachment> {
  const response = await fetch('/api/app-attachments/sign-upload', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${idToken}`,
...
export async function getAppAttachmentDownloadUrl(storagePath: string, idToken: string): Promise<string> {
  const response = await fetch('/api/app-attachments/sign', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${idToken}`,
...
export async function downloadAppAttachment(storagePath: string, filename: string, idToken: string, fallbackUrl?: string) {
  let url = fallbackUrl;
  if (!url) {
    url = await getAppAttachmentDownloadUrl(storagePath, idToken);
  }
  try {
    const response = await fetch(url);
    if (!response.ok) {
...
export async function deleteAppAttachment(storagePath: string, idToken: string) {
  const response = await fetch('/api/app-attachments/delete', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${idToken}`,
